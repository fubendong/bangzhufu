...
(function($){"use strict";$.extend($.fn.select2.defaults,{formatNoMatches:function(){return"没有找到匹配项";},formatInputTooShort:function(input,min){var n=min-input.length;return"请再输入"+n+"个字符";},formatInputTooLong:function(input,max){var n=input.length-max;return"请删掉"+n+"个字符";},formatSelectionTooBig:function(limit){return"你只能选择最多"+limit+"项";},formatLoadMore:function(pageNumber){return"加载结果中...";},formatSearching:function(){return"搜索中...";}});})(jQuery);;
 ...
 * Date:     3rd Feb 2013
 */
(function($){$.fn.areYouSure=function(options){var settings=$.extend({'message':'You have unsaved changes!','dirtyClass':'dirty','change':null,'fieldSelector':"select,textarea,input[type='text'],input[type='password'],input[type='checkbox'],input[type='radio']"},options);var getValue=function($field){if($field.hasClass('ays-ignore')||$field.hasClass('aysIgnore')||$field.attr('data-ays-ignore')){return null;}
var val;var type=$field.attr('type');if($field.is('select')){type='select';}
switch(type){case'checkbox':case'radio':val=$field.is(':checked');break;case'select':val='';$field.children('option').each(function(o){var $option=$(this);if($option.is(':selected')){val+=$option.val();}});break;default:val=$field.val();}
return val;};var storeOrigValue=function(){var $field=$(this);$field.data('ays-orig',getValue($field));};var checkForm=function(evt){var isFieldDirty=function($field){return(getValue($field)!=$field.data('ays-orig'));};var isDirty=false;var $form=$(this).parents('form');if(isFieldDirty($(evt.target))){isDirty=true;}
if(!isDirty){$form.find(settings.fieldSelector).each(function(){$field=$(this);if(isFieldDirty($field)){isDirty=true;return false;}});}
var changed=isDirty!=$form.hasClass(settings.dirtyClass);$form.toggleClass(settings.dirtyClass,isDirty);if(changed&&settings.change){settings.change.call($form,$form);}};$(window).bind('beforeunload',function(){$dirtyForms=$("form").filter('.'+settings.dirtyClass);if($dirtyForms.length>0){return settings.message;}});return this.each(function(elem){if(!$(this).is('form')){return;}
$(this).submit(function(){$(this).removeClass(settings.dirtyClass);});$(this).find(settings.fieldSelector).each(storeOrigValue);$(this).find(settings.fieldSelector).bind('change keyup',checkForm);});};})(jQuery);;
...
this.isShown=true
this.escape()
if(this.options.dynamic){this.$elementWrapper=$('<div class="modal-wrapper" />').insertBefore(this.$element)
...
that.enforceFocus()
transition?that.$element.one($.support.transition.end,function(){that.$element.trigger('shown')}):that.$element.focus().trigger('shown')})},hide:function(e){e&&e.preventDefault()
var that=this
e=$.Event('hide')
...
if(this.options.dynamic){this.$element.insertAfter(this.$elementWrapper)
this.$elementWrapper.remove()
this.$elementWrapper=null}
this.backdrop()},removeBackdrop:function(){this.$element.insertAfter(this.$backdrop)
this.$backdrop.remove()
this.$backdrop=null
$('body').css({'overflow':''})},backdrop:function(callback){var that=this,animate=this.$element.hasClass('fade')?'fade':''
if(this.isShown&&this.options.backdrop){var doAnimate=$.support.transition&&animate
this.$backdrop=$('<div class="modal-backdrop '+animate+'" />').appendTo(document.body)
if(!that.$element.parent().length){this.$backdrop.appendTo(document.body)}else if(this.options.dynamic){this.$backdrop.insertBefore(this.$elementWrapper)}else{this.$backdrop.insertBefore(this.$element)}
if(this.options.dynamic){this.$elementWrapper.prependTo(this.$backdrop).delegate('[data-dismiss="modal"]','click.dismiss.modal',$.proxy(this.hide,this))}else{this.$element.prependTo(this.$backdrop).delegate('[data-dismiss="modal"]','click.dismiss.modal',$.proxy(this.hide,this))}
$('body').css({'overflow':'hidden'})
this.$backdrop.on('click',function(e){if(that.options.backdrop=='static'){$.proxy(that.$element[0].focus,that.$element[0])}else if(e.target==e.delegateTarget){that.hide(e)}})
...
{this.fini=true;var ie=0//@cc_on + @_jscript_version
...
$.fn.clickover.defaults=$.extend({},$.fn.popover.defaults,{trigger:'manual',auto_close:0,global_close:1,esc_close:1,onShown:null,onHidden:null,width:null,height:null,tip_id:null,class_name:'clickover',allow_multiple:0})}(window.jQuery);;


/**  bootstrap-custom/plugin/pretty-loader/js/jquery.prettyLoader.js  **/

var prettyLoaderRoot='';if(WEB_ROOT_BASE_RES){prettyLoaderRoot=WEB_ROOT_BASE_RES+'bootstrap-custom/plugin/pretty-loader';}
...
var bZF={};if(!window.bZF){window.bZF=bZF;}else{bZF=window.bZF;}
bZF.isWindowUnload=false;$(window).bind('beforeunload',function(){bZF.isWindowUnload=true;});(function(targetObj){window.bzFWETEWXX_isIE6=false;document.write("<!--[if lt IE 7]><script>bzFWETEWXX_isIE6=true;</script><![endif]-->");targetObj.isIE6=bzFWETEWXX_isIE6;window.bzFWETEWXX_isIE7=false;document.write("<!--[if lt IE 8]><script>bzFWETEWXX_isIE7=true;</script><![endif]-->");targetObj.isIE7=bzFWETEWXX_isIE7;})(bZF);})(window,jQuery);jQuery((function(window,$){if($.pnotify){$.pnotify.defaults.history=false;$.pnotify.defaults.delay=4000;}
...
return fmt;};bZF.slideToggle=function(itemId){$(itemId).slideToggle();};bZF.makeUrl=function(url){return WEB_ROOT_HOST+WEB_ROOT_BASE+url;};bZF.ajaxCallGet=function(callUrl,successFunc,failFunc){$.ajax({type:"get",url:callUrl,dataType:"json",success:function(result){if(result.error){if(result.error.message){bZF.showMessage(result.error.message);}else{bZF.showMessage('调用失败');}
return;}
...
successFunc(result.data);},error:function(jqXHR,textStatus,errorThrown){if(failFunc){failFunc(jqXHR,textStatus,errorThrown);return;}
if(bZF.isWindowUnload){return;}
bZF.showMessage('网络错误['+textStatus+']');}});};bZF.showMessage=function(message,type){if(!type){type='success';}
$.pnotify({text:message,type:type,delay:3000,before_open:function(pnotify){pnotify.css({"top":($(window).height()/2)-(pnotify.height()/2),"left":($(window).width()/2)-(pnotify.width()/2)});}});console.log('['+new Date().pattern("yyyy-MM-dd HH:mm:ss")+']'+message);};(function($){var breadCrumbKey='bzf_breadCrumbKey';bZF.setBreadCrumb=function(index,text,link){var breadCrumbArray=$.jStorage.get(breadCrumbKey);if(!breadCrumbArray){breadCrumbArray=[];}
breadCrumbArray=breadCrumbArray.slice(0,index);breadCrumbArray.push({index:index,text:text,link:link});$.jStorage.set(breadCrumbKey,breadCrumbArray);};if(window.bz_set_breadcrumb_status){for(var i=0;i<window.bz_set_breadcrumb_status.length;i++){var item=window.bz_set_breadcrumb_status[i];bZF.setBreadCrumb(item.index,item.text,item.link);}}
var breadCrumbArray=$.jStorage.get(breadCrumbKey);if(!breadCrumbArray||breadCrumbArray.length<=1){return;}
var htmlStr='';for(var i=0;i<breadCrumbArray.length;i++){var item=breadCrumbArray[i];htmlStr+='<li><a href="'+item.link+'">'+item.text+'</a><span class="divider">/</span></li>';}
htmlStr='<ul class="breadcrumb">'+htmlStr+'</ul>';$('#main_body_breadcrumb').html(htmlStr);$('#main_body_breadcrumb').show();})(jQuery);bZF.loadCaptchaImage=function(itemId){var time=new Date().getTime();$(itemId).html('<a href="#" onclick="bZF.loadCaptchaImage(\''+itemId+'\')"><img style="width:150px;height:50px;" '
+'width="150" height="50" src="'+bZF.makeUrl('/Image/Captcha')+'?rand='+time+'" /></a>');if(bZF.isIE6||bZF.isIE7){$('img',$(itemId)).width(150);$('img',$(itemId)).height(50);}};(function($){var oldConfig=$.cookie.json;$.cookie.json=true;var flashMessageStr=$.cookie("flash_message");$.cookie.json=oldConfig;if(!flashMessageStr){return;}
var msgHtml='';$.each(flashMessageStr,function(i,item){msgHtml+='<li>'+item+'</li>';});msgHtml='<ul>'+msgHtml+'</ul>';bZF.showMessage(msgHtml);$.removeCookie("flash_message",{path:WEB_ROOT_BASE+'/'});})(jQuery);(function($){if(window.bz_set_nav_status){$.each(window.bz_set_nav_status,function(i,value){value($);});}})(jQuery);if($.prettyLoader){$.prettyLoader();}
bZF.enhanceHtml=function(node){$("input,select,textarea",node).not('[type="image"],[type="submit"],[data-no-validation="true"],.select2-input,.select2-simple,.select2-simple-tag,.ke-edit-textarea,.editor-html-simple').jqBootstrapValidation({filter:function(){if($(this).attr("data-no-validation")||$(this).hasClass('select2-input')||$(this).hasClass('select2-simple')||$(this).hasClass('ke-edit-textarea')||$(this).hasClass('editor-html-simple')){return false;}
return true;}});$('select.select2-simple',node).each(function(index,elem){$(elem).select2({minimumResultsForSearch:10,allowClear:true}).select2('val',$(elem).attr('data-initValue'));if($(elem).attr('data-ajaxCallUrl')){var callUrl=$(elem).attr('data-ajaxCallUrl');var valueKey=$(elem).attr('data-option-value-key');var textKey=$(elem).attr('data-option-text-key');bZF.ajaxCallGet(callUrl,function(data){if(!data){return;}
$.each(data,function(index,dataItem){$(elem).append($('<option value="'+dataItem[valueKey]+'">'+dataItem[textKey]+'</option>'));});$(elem).select2('val',$(elem).attr('data-initValue'));});}});$('div.datetimepicker',node).datetimepicker({format:'yyyy-MM-dd hh:mm:ss',todayHighlight:true,language:'zh-cn'});$('textarea.editor-html-simple',node).each(function(index,elem){KindEditor.create(elem,{filterMode:true,themeType:'default',cssData:"body {font-family: '微软雅黑', 'Microsoft Yahei', '宋体', 'songti', STHeiti, Helmet, Freesans, sans-serif;font-size: 15px; }",width:$(elem).outerWidth(false),items:['removeformat','fontsize','forecolor','hilitecolor','bold','italic','underline','justifyleft','justifycenter','justifyright','insertorderedlist','insertunorderedlist','link']});});$('form.form-dirty-check',node).areYouSure({'message':'内容发生修改，你确定不保存就离开？'});$('*[rel="tooltip"]',node).tooltip();$("a[ref='popover']",node).popover();$('a[rel="clickover"]',node).clickover({html:true,template:'<div class="popover"><div class="arrow"></div><div class="popover-inner"><div class="popover-content"><p></p></div></div></div>'});$("img.lazyload",node).show().lazyload({effect:"fadeIn"});};bZF.enhanceHtml(document);bZF.Order_ListOrder_Detail=function(rec_id){var ajaxCallUrl=bZF.makeUrl('/Order/Goods/ajaxDetail');$('#order_detail_dialog').load(ajaxCallUrl+'?rec_id='+rec_id,function(){bZF.enhanceHtml($('#order_detail_dialog'));$('#order_detail_dialog').modal({dynamic:true});});};bZF.Order_ListOrder_ajaxUpdate=function(){var ajaxCallUrl=bZF.makeUrl('/Order/Goods/ajaxUpdate');var rec_id=$('#order_detail_rec_id').val();var shipping_id=$('#order_detail_shipping_select').find('option:selected').val();var shipping_no=$('#order_detail_shipping_no').val();bZF.ajaxCallPost(ajaxCallUrl,{rec_id:rec_id,shipping_id:shipping_id,shipping_no:shipping_no},function(data){bZF.showMessage('快递信息设置成功');});};})(window,jQuery));;
